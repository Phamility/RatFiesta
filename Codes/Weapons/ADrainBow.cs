...
{
    public class ADrainBow : ModItem
    {
...
//Per usage, inflict 4 damage onto player (Ignores Defense)
            player.Hurt(PlayerDeathReason.ByCustomReason(player.name + " turned to gel!"), (int)Math.Round(FinalBooga + player.GetModPlayer<ExamplePlayer>().EnduranceForDrainBow1), 0);
            player.immune = false;
            player.immuneTime = 0;
            return true;
